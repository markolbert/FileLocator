...
public static partial class TypeLogExtensions
{
    ...
    public static partial void InstanceNotRetrieved(this ILogger logger, Type type, [CallerMemberName] string caller = "");

    [LoggerMessage(LogLevel.Critical, Message = "{caller}: Could not create instance of {type}, message was '{mesg}'")]
    public static partial void InstanceCreationFailed(this ILogger logger, Type type, string mesg, [CallerMemberName] string caller = "");

    [LoggerMessage(LogLevel.Critical, Message = "{caller}: No matching public constructor found")]
    ...
    public static partial void UnexpectedType(this ILogger logger, Type type, Type badType, [CallerMemberName] string caller = "");

    [LoggerMessage(Level = LogLevel.Error, Message = "{caller}: {type} is unsupported{retVal}")]
    public static partial void UnsupportedType(this ILogger logger, Type type, string retVal, [CallerMemberName] string caller = "");

    [LoggerMessage( Level = LogLevel.Warning, Message = "{caller}: {type} is not configured" ) ]
    public static partial void UnconfiguredInstance( this ILogger logger, Type type , [CallerMemberName] string caller = "");
